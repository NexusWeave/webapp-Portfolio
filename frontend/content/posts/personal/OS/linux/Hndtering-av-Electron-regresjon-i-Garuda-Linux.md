...
star: |
  ...
  Etter en nylig oppdatering i <abbr title ="Et redigerings verktøy for programmering">`visual-studio-code-bin `(v1.107), i  **Garuda Dr460nized Gaming**-**distribusjon**, oppsto det en uforutsett feil som deaktiverte hovedmenyen i VS Code. Dette gjorde prosjektnavigasjonen og bruken av menyvalgene umulig. Diagnosen indikerte en inkompatibilitet mellom **VS Code** og <abbr title="Det visuelle miljøet på maskinen (knapper, menyer og vinduer).">**KDE Plasma**</abbr>, spesifikt knyttet til hvordan <abbr title="Lego klosse som mange moderne programmer bruker for å vise vinduer og menyer.">**Electron**</abbr>-biblioteket håndterer rendring av globale menyer i skrivebordsmiljøet.
  ...
  * Jeg benyttet verktøyet <abbr title="Et verktøy som lar deg velge nøyaktig hvilken tidligere versjon av et program du vil installere.">`downgrade`</abbr> for  å avinstallere nåværende versjon av pakken og installere tilbake til en tidligere versjon. Siden dette er et Arch-basert system, ble følgende system kommando brukt `sudo downgrade visual-studio-code-bin` til den den stabile versjonen (`v.1.106`) som ikke hadde den spesifikke Electron-utfordringen knyttet til de globale menyer i KDE Plasma.
