...
describe ("Article header renders correctly", () => {

    it("Renders suspense correctly with HTML & classes not as post", async() => {

        const wrapper = await mountSuspended<Component>(Head, { props: {isPost: false, article: dummyPostItem }, global:{ stubs:{ MDC: { props: ['value'], template: '<div class="ingress-content"><slot /></div>' }, NavigationNavMenu: true }} });
        await flushPromises();
    ...
    it("Renders suspense correctly with classes as post", async() => {
        const wrapper = await mountSuspended<Component>(Head, { props: {isPost: true, 
            article: dummyHeaderData }, global:{ stubs:{ 
                MDC: { props: ['value'], 
                    template: '<div class="ingress-content"><slot /></div>' }, 
...
describe ("Article body renders correctly", () => {

    it("Renders Suspense correctly with HTML & CSS", async() => {
        const wrapper = await mountSuspended<Component>(Body, { props: { data: dummyBodyData[0] }, global: {  stubs: { MDC: { props: ['value'], template:'<div><slot /></div>' }, ContentRenderer: { props: ['value'], template:'<div><slot /></div>' } }}});
        await flushPromises();
    ...
    it("Renders Suspense correctly with CTA only", async() => {
        const wrapper = await mountSuspended<Component>(Body, { props: { data: dummyBodyData[1] }, global: {  stubs: { MDC: { props: ['value'], template:'<div><slot /></div>' }, ContentRenderer: { props: ['value'], template:'<div><slot /></div>' } }}});
        await flushPromises();
    ...
    it("Renders Suspense correctly with media only", async() => {
        const wrapper = await mountSuspended<Component>(Body, {props: { data: dummyBodyData[2] }, global: {  stubs: { MDC: { props: ['value'], template:'<div><slot /></div>' }, ContentRenderer: { props: ['value'], template:'<div><slot /></div>' } }}});
        await flushPromises();
    ...
    it("Renders Suspense correctly with minimal version only", async() => {
        const wrapper = await mountSuspended<Component>(Body, { props: { data: dummyBodyData[3] }, global: {  stubs: { MDC: { props: ['value'], template:'<div><slot /></div>' }, ContentRenderer: { props: ['value'], template:'<div><slot /></div>' } }}});
        await flushPromises();
...
describe ("Article page renders correctly", () => {
    ...
    it("Renders tag page correctly with classes", async() => {
        ...
        const wrapper = await mountSuspended<Component>(Page, { props: { data: dummyPageData[2] }, route: { name: 'logs-tags-slug', params: {slug: 'vue'} }, global: { stubs: { ArticleHead: true, ArticleBody: true, NavigationButton: { template: '<button class="primary-btn"><slot /></button>' } }}});
        await flushPromises();
