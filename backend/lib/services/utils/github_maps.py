...
class ServicesUtils:
    ...
    async def map_repo(self, data: Dict[str, str | object], languages: Dict[str, str | int], collaborators: Optional[List[Dict[str, str | object]]] = None) -> Dict[str, str | object | List[str] | object]:
        ...
        track_stack = await self.track_project_stack(str(data['default_branch']), str(data['trees_url']), n=1)
    ...
    async def track_project_stack(self, branch:str, tree_path:str, n:int = 1) -> Dict[str, bool]:
        ...
        tree: List[Dict[str, Any]] = repo_tree.get('tree', [])
        if not tree:
        ...
        for item in tree:
            ...
            if not dictionary.get('is_backend'):
                dictionary['is_backend'] = self.check_backend_frontend(path, backend_extensions)
            if not dictionary.get('is_frontend'):
                dictionary['is_frontend'] = self.check_backend_frontend(path, frontend_extensions)
    ...
    def check_backend_frontend(self, file: str, extensions: List[str]) -> bool:
        file = file.lower()
